...
const App = () => {
  ...
  return (
    <div className={`transition-all duration-700 min-h-screen font-inter ${
      ...
      {/* Pass darkMode to Navbar so it can flip text colors */}
      ...
      <section id="about" className="py-24 px-6 md:px-20 grid md:grid-cols-2 gap-12 items-center">
        ...
        <div data-aos="fade-left" className="relative flex justify-center items-center">
            {/* Decorative background circle (Yin-Yang vibe) */}
      ...
      <section id="skills" className={`py-24 px-6 md:px-20 ${darkMode ? 'bg-zinc-900' : 'bg-gray-100'} transition-colors`}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
          {['React', 'Tailwind', 'TypeScript', 'Next.js', 'AOS', 'Git'].map((skill, i) => (
            <div key={skill} data-aos="flip-up" data-aos-delay={i * 100} className={`p-8 border ${darkMode ? 'border-zinc-700 hover:bg-white hover:text-black' : 'border-gray-300 hover:bg-black hover:text-white'} transition-all cursor-default text-center font-syne font-bold uppercase`}>
              {skill}
            </div>
          ))}
        </div>
      ...
      <footer id="contact" className={`py-20 text-center transition-colors ${darkMode ? 'bg-white text-black' : 'bg-black text-white'}`}>
        ...
        <p className="mt-8 font-inter uppercase tracking-widest font-bold">Christianbasingan14@gmail.com</p>
